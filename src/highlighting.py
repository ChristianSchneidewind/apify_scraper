def _build_highlight_prelude_js() -> str:
    return """
    (payload) => {
...
def _build_detached_fallback_js() -> str:
    return """
...
def _build_row_selection_js() -> str:
    return """
      let row = null;
      if (isPostPage) {
...
def _build_apply_highlight_js() -> str:
    return """
...
HIGHLIGHT_COMMENT_JS = "\n".join(
    [
        _build_highlight_prelude_js(),
        _build_detached_fallback_js(),
        _build_row_selection_js(),
        _build_validation_js(),
        _build_apply_highlight_js(),
    ]
)


def build_highlight_payload(*, element_handle, comment_data: dict) -> dict:
    return {
        "el": element_handle,
